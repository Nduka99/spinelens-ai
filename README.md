...
The figures behind the evidence above, generated by the notebooks in `phase1_spinelens_ai/`. The full set lives under `phase1_spinelens_ai/outputs/reports/`.

**Route legibility**

![Route Legibility Index by approach with its five weighted components](phase1_spinelens_ai/outputs/reports/route_legibility_media/figB_rli_components.png)
*Route Legibility Index for each approach and the five components behind it. Colmore Row reads easiest, Snow Hill hardest.*

![RLI distribution and rank probability over 4,000 random weightings](phase1_spinelens_ai/outputs/reports/route_legibility_media/figE_weight_montecarlo.png)
*The ranking re-run 4,000 times with random weightings (Dirichlet Monte Carlo). Colmore is rank 1 in 86 percent of runs, so the order is robust, not cherry-picked.*

**Wayfinding**

![Wayfinder coverage curve and before/after legibility](phase1_spinelens_ai/outputs/reports/wayfinder_media/figH_coverage_and_beforeafter.png)
*Diminishing returns: about seven wayfinders cover roughly 90 percent of the city-centre confusion (12 in total once the Nechells side is added). Before and after wayfinding, the hardest routes gain the most.*

**The crossing**

![Crossing evidence dashboard: traffic, cycle share and collisions](phase1_spinelens_ai/outputs/reports/crossing_media/figJ_evidence_dashboard.png)
*Dartmouth and Jennens: about 35,000 vehicles a day against 43 bikes, with a measured injury history. The case for a signalised crossing rather than paint.*

**The amber corridor**

![Tactical corridor intensity ramping toward the gateway](phase1_spinelens_ai/outputs/reports/corridor_media/figK_corridor_intensity.png)
*The city-centre approaches converge into one short shared trunk, so the amber route can run near-continuous and low-cost.*

**The gateway pavilion**

![Pavilion suitability score and hard risk gate](phase1_spinelens_ai/outputs/reports/pavilion_media/figO_suitability_risk.png)
*A data-backed suitability score of about 76 percent, gated by a hard safeguarding and ownership check.*

**Budget**

![Phase 1 everything-in cost vs the one million pound envelope](phase1_spinelens_ai/outputs/reports/budget_media/figU_phase1_vs_envelope.png)
*Low, central and high estimates against the 1 million pound budget. The central estimate fits with headroom; the high estimate does not, kept visible on purpose.*

**At a glance**

![Phase 1 evidence summary: legibility, budget, validation register and data by evidence level](phase1_spinelens_ai/outputs/reports/evidence_pack_media/figV_evidence_at_a_glance.png)
*One slide: legibility, budget vs the envelope, the open validation items, and data sources by evidence level (most at Level 3, audited).*
